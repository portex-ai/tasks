...
def write_reward(
    ...
    """Write a full ``reward.json`` with the Harbor-expected schema."""
    total_score = normalize_score(total_score_raw)
    passed = check_passed(total_score_raw, pass_threshold)

    reward = {
        "total_score": total_score,
        "portex_passed": passed,
        "portex_detail": {
            "task_id": task_id,
...
def write_error_reward(reward_path: str, error_message: str) -> None:
    """Write a minimal ``reward.json`` indicating an error (``total_score: 0.0``)."""
    reward = {
        "total_score": 0.0,
        "error": error_message,
    }
